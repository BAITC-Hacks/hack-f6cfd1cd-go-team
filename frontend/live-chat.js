...
export function initLiveChat({ openChat, scrollChat, message }) {
  const session = createChatSession(); let generation = 0, cartState;
  ...
  $('.chat-safety').textContent = 'Demo-корзина в чате — не настоящая корзина ekt.kz. Подтверждение проверяет сервер.';
  ...
  async function send(text) {
    ...
    try {
      ...
      for (const analog of result.analogs) {
        const card = productCard(analog.product);
        card.prepend(el('strong', 'chat-extra-title', `Возможный аналог товара №${analog.sourceProductId}`));
        card.append(el('p', 'chat-stock', analog.explanation), el('p', 'chat-stock', analog.disclaimer));
      }
      cartState?.remove();
      cartState = renderSessionCart(result);
      if (cartState) log.append(cartState);
    } catch (error) {
      if (turn !== generation || error.code === 'cancelled') return;
      cartState?.remove(); cartState = null;
...
// Render only the server snapshot; confirmations remain ordinary chat messages.
export function renderSessionCart({ cart, pendingConfirmation }) {
  if (!cart && !pendingConfirmation) return null;
  const panel = el('section', 'chat-session-cart');
  panel.append(el('strong', 'chat-extra-title', 'Demo-корзина диалога'), el('p', '', 'Не настоящая корзина ekt.kz. Заказ не оформляется. Содержимое хранится только в этой сессии.'));
  if (cart) {
    if (!cart.items.length) panel.append(el('p', 'chat-stock', 'Корзина пуста.'));
    for (const item of cart.items) panel.append(el('p', 'chat-stock', `${item.name} · Артикул: ${item.article} · В корзине: ${item.quantity} шт.`));
  }
  if (pendingConfirmation) {
    panel.append(el('strong', 'chat-extra-title', 'Ожидает подтверждения — ещё не добавлено'),
      el('p', '', `Артикул: ${pendingConfirmation.article} · К добавлению: ${pendingConfirmation.quantity} шт.`),
      el('p', 'chat-stock', 'Отправьте «Да, добавь» для подтверждения или «Отмена» для отмены. Проверку наличия и добавление выполняет сервер.'));
  }
  return panel;
}
